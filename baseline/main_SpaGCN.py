...
BASE_PATH = Path('')
output_path = Path('/output/SpaGCN_results')
...
for sample_name in sample_list:
    ...
    spatial = pd.read_csv(f"{dir_input}/spatial/tissue_positions_list.csv", sep=",", header=None, na_filter=False,
                          index_col=0)
    ...
    print(f"{dir_input}/spatial/full_image.tif")
    ...
    b = 49
    a = 1
    adj = spg.calculate_adj_matrix(x=x_pixel, y=y_pixel, x_pixel=x_pixel, y_pixel=y_pixel, image=img, beta=b, alpha=a,
                                   histology=True)
    ...
    methods = 'SpaGCN'
    results_df = calculate_clustering_matrix(df, df_meta['SpaGCN'], df_meta['layer_guess'], sample_name, methods)
    results_df.to_csv('SpaGCN_151509.csv')
